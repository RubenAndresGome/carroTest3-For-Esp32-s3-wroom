# Documentación

Este directorio reúne la documentación vigente del proyecto.

- [Protocolo de pasos v2 entre HMI y ESP32](protocolo_json_steps_v2_hmi_esp32.md)
- [Validación física del sistema final](validacion_sistema_final.md)
- [Archivo histórico de pruebas](../archive/firmware_tests/READMEForTests.md)
- [Desarrollo del frontend](desarrollo_frontend.md)
- [Manual de ensamble físico](hardware/manual_ensamble_fisico.md)
- [Diagrama de ensamble](hardware/diagrama_ensamble.md)
- [Diagrama interactivo](hardware/diagrama_interactivo/diagrama.html)

Los documentos de iteraciones anteriores están conservados en
[`archive/legacy`](../archive/legacy/README.md) y no describen necesariamente
la versión activa.
