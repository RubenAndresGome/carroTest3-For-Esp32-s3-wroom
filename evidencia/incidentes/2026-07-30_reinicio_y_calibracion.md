...
|---|---:|---|
| 00:12:11 | S-A | WebSocket conectado al destino local configurado, firmware `robot-s3-v3`, protocolo `steps-v3`. |
| 00:12:17 | S-A | `calibrate`, `seq=1`, aceptado. |
| 00:12:44 | S-A | `completed`, `detail=cal_ok`; telemetría posterior `listo`, `cal=true`, `last_seq=1`. |
| 00:13:04 | S-A | `BACKOFF`, `Software caused connection abort`. |
| 00:13:08 | S-B | WebSocket reconectado. La sesión corta Python conservó la misma identidad (valor oculto). |
| 00:13:08 | S-B | Primera telemetría: `desarmado`, `cal=false`, `last_seq=0`, pose `(0,0)`, `reset_reason=power_on`. |
| 00:14:00 | — | `settings.active_mission=null`; ningún comando `step` fue registrado. |
| 00:15:12 | S-B | Segundo `calibrate`, `seq=2`, aceptado. |
| 00:15:23 | S-B | `fault`, `detail=cal_stall_left`; telemetría en fallo con PWM `0/0`. |
| 00:15:41 | S-B | Nueva caída de transporte seguida de `Network is unreachable`. |
