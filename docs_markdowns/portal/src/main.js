  ...
  "seq": 7
      ...
      <section id="evidencia" class="section">
        ...
        <div class="mt-8 grid gap-8 xl:grid-cols-2">
          ${evidence.map((item) => `<article class="video-card card"><div class="flex items-start justify-between gap-4"><div><h3 class="text-xl font-semibold">${item.title}</h3>${item.subtitle ? `<p class="mt-1 text-xs font-semibold text-emerald-400"><span class="inline-block rounded bg-emerald-500/10 px-2 py-0.5 border border-emerald-500/20">${item.subtitle}</span></p>` : ""}<p class="mt-1 font-mono text-xs text-slate-500">${item.duration} · SHA ${item.hash}</p></div><img class="h-28 w-20 rounded-lg object-cover" src="${assetPath(item.thumbnail)}" alt="Miniatura animada de ${item.title}"></div><video class="mt-5" controls preload="metadata"><source src="${encodeURI(assetPath(item.src))}" type="video/mp4" />Tu navegador no puede reproducir este video.</video><div class="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">${item.frames.map(([time, src, caption]) => `<figure><img class="timeline-img" loading="lazy" src="${assetPath(src)}" alt="${caption} a ${time}" /><figcaption class="mt-2 text-xs text-slate-500"><span class="font-mono text-cyan">${time}</span> · ${caption}</figcaption></figure>`).join("")}</div></article>`).join("")}
        </div>
      </section>
