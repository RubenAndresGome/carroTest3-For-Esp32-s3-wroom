...
static ControlInicializacionPCNT::Canal setup_PCNT(int pin, pcnt_unit_t unit) {
    ...
    // El filtro de 1023 ciclos APB (12.8 us) rechaza transitorios y rebotes inductivos de motor sin perder pulsos legítimos.
...
void setup_Sensores() {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        Serial.printf("PCNT %s: %s etapa=%s error=%d\n", nombres[i],
